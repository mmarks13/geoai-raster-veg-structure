...
# Forest plot site bboxes (WGS84, for 3DEP caching)
FOREST_PLOT_SITE_BBOXES = {
    "BluffMesa": (-116.95914853914289, 34.21528660609054, -116.95176244347506, 34.222233606043176),
    "Laguna": (-116.4381379772187, 32.84449833409179, -116.42423938500946, 32.86224048604598),
    "NorthBigBear": (-116.937284989011, 34.28765157315019, -116.91803715530904, 34.304256927475734),
    "ReyesPeak": (-119.34136607196854, 34.631433760919556, -119.2822768521021, 34.644352109514294),
    "TecuyaRidge": (-119.02544697505307, 34.839899976415694, -118.96743505454289, 34.84850983435747),
}

# Choose which area a tile belongs to
def choose_area_bbox(tile_bbox, tile_properties=None):
    """
...
def create_stac_stack(bbox, start_date, end_date, stac_source, 
    ...
    try:
        ...
        for item in catalog.get_all_items():
            ...
            elif 'start_datetime' in item.properties and item.properties['start_datetime'] is not None:
                # Use start_datetime if datetime is None
                from datetime import datetime as dt
                start_dt = dt.fromisoformat(item.properties['start_datetime'].replace('Z', '+00:00'))
                item_date = start_dt.date()
            else:
                # Skip items without datetime information
                if verbose:
                    print(f"Skipping item {item.id} - no datetime information")
                continue
...
def create_3dep_stack(tile_bbox, start_date, end_date, area_bbox, threads=8, target_crs=None, stac_source=None):
    """
    ...
    stac_source : str, optional
        Path to local 3DEP STAC catalog. If None, will query Planetary Computer API.
        Note: Local catalogs are often very large. Use only if you have pre-downloaded
        and cropped tiles for your study area using make_local_3dep_stac.py.
    ...
    try:
        ...
        if stac_source:
            ...
            # Use Planetary Computer API (original behavior)
...
def process_bbox(args):
    ...
    try:
        ...
        # Step 1: UAV LiDAR point clouds (skip if not needed for test data)
        if skip_uav_lidar:
            if verbose:
                print(f"Skipping UAV LiDAR for tile {tile_id} (test-only mode)")
            xyz_uav = np.empty((0, 3), dtype=np.float32)  # Empty array
            # Empty attribute array matching the format (N, 3) used when UAV data is present
            uav_pnt_attr = np.empty((0, 3), dtype=np.float32)
            uav_meta = None  # No UAV metadata when skipping
            # Create empty downsampling masks and voxel sizes
            downsample_masks = []
            voxel_sizes = []
        ...
        with ProcessPoolExecutor(max_workers=max_threads, mp_context=mp.get_context('spawn')) as executor:
            future_to_tile = {
                ...
                for i, (args, (i, tile_id)) in enumerate(
                    zip(args_list, enumerate([tid for tid, _, _ in chunk], start=1))
                )
            }
                ...
                for batch_start in range(0, len(chunk_results), batch_size):
                    ...
                    for i, result in enumerate(batch):
                        ...
                        for key, value in result.items():
                            ...
                            elif key in ['uav_meta', 'dep_meta'] or key.endswith('_data_stack'):
                                # Skip complex metadata and xarray objects - will be handled separately
                                continue
                        ...
                        for key in ['uav_points', 'uav_pnt_attr', 'dep_points', 'dep_pnt_attr']:
                            if key in result:
                                data = result[key]
                                # Ensure data is not object dtype
                                if isinstance(data, np.ndarray) and data.dtype == object:
                                    print(f"Warning: {key} has object dtype after sanitization, skipping")
                                    continue
                                pc_group.create_dataset(key, data=data, compression='gzip')
...
def process_tiles_from_geojson(
    ...
    # Validate CRS - must be EPSG:32611 (WGS84 UTM Zone 11N)
    expected_crs = "EPSG:32611"
    actual_crs = str(gdf.crs)
    if actual_crs != expected_crs:
        raise ValueError(
            f"\n❌ CRITICAL CRS MISMATCH!\n"
            f"Input tiles file: {tiles_geojson}\n"
            f"Expected CRS: {expected_crs}\n"
            f"Actual CRS: {actual_crs}\n\n"
            f"Input tiles MUST be in EPSG:32611 (WGS84 UTM Zone 11N).\n"
            f"Reproject the input file before processing.\n"
            f"Do NOT proceed with mismatched CRS - this will cause coordinate errors."
        )
    ...
    # Extract bounding boxes and properties from tile geometries
    tile_bounding_boxes = []
    tile_ids = []  # Track tile IDs if available, otherwise use index
    tile_properties_list = []  # Track tile properties for site-based processing
    ...
    # Split tiles into chunks (now includes properties)
    chunks = list(chunk_data(list(zip(tile_ids, tile_bounding_boxes, tile_properties_list)), chunk_size))
    ...
    if retry_failed and results_tracker['failed']:
        ...
        # Extract the failed tile IDs and find their corresponding bounding boxes and properties
        failed_tiles = []
        original_tile_id_to_bbox = dict(zip(tile_ids, tile_bounding_boxes))
        original_tile_id_to_props = dict(zip(tile_ids, tile_properties_list))

        for tile_id, _ in results_tracker['failed']:
            # Get the bounding box and properties for this tile ID
            if tile_id in original_tile_id_to_bbox:
                failed_tiles.append((tile_id, original_tile_id_to_bbox[tile_id], original_tile_id_to_props[tile_id]))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Path to UAV LiDAR STAC catalog"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to local 3DEP STAC catalog (optional). If not provided, will query Planetary Computer API directly. Note: Local 3DEP catalogs can be very large; use only if you have pre-downloaded cropped tiles for your study area."
    )
