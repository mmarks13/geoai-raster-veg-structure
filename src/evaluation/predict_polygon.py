...
def run_step(cmd: List[str], step_name: str) -> None:
    """Run a sub-pipeline command, streaming output; raise on non-zero exit."""
    logger.info("=" * 70)
...
def build_inference_tiles(
    ...
    work_dir: Path, min_dep_points: int,
) -> Path:
    ...
    geometry). All six 3DEP attributes are preserved end-to-end.
    """
...
def finalize_outputs(
    ...
    name: str, band_config_path: str, invocation: dict,
) -> None:
    ...
    vrt_path = per_band_dir / f"{name}.vrt"
...
def process_aoi(aoi_geom, name: str, args, fuel_stats: str) -> None:
    ...
    # 4) per-tile build → inference-ready
    ...
    run_inference(
        ...
        args.mc_samples, args.batch_size, args.multi_gpu, args.num_gpus, args.device,
    )
    # 6) dedup + stitch
    pt, csv, std_pt = dedup_predictions_csv(preds_dir)
    rasters_dir = work_dir / "site_rasters"
    mean_tif, std_tif = stitch_rasters(pt, csv, std_pt, rasters_dir, name)
    # 7) finalize
    finalize_outputs(mean_tif, std_tif, aoi_geom, out_dir, name, args.band_config, invocation)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--mc-samples", type=int, default=30,
                   help="MC-dropout samples (emits matching *_std bands).")
